...
import { FaCog, FaCheck, FaArrowDown } from "react-icons/fa";
...
export class ProductsComponent extends Component {
  ...
  handleChange = (event, form = "create") => {
    let { createProductForm, updateProductForm, createStockForm, keywords } =
      this.state;
  ...
  handleDelete = (i) => {
    ...
    if (!keywords[i].isPseudo) {
      // pseudos are new(manual) additions. You can only delete manual additions
      return;
    }
    this.setState({
   ...
   */
  createProduct = async () => {
    let { createProductForm, allProducts, keywords, authUser } = this.state;
    let isSaving = true;
    let saveMsg = "Saving";
    this.setState({ isSaving, saveMsg });
    createProductForm["search_path"] = this.getSearchPath(keywords);
    createProductForm["category"] = this.getCategoryById(
      createProductForm["category"]
    );
    createProductForm["keywords"] = keywords;
    createProductForm["no_of_stocks"] = 0;
    createProductForm["owner"] = authUser._id;
    this.appMainService
      .createProduct(createProductForm)
      .then((productData) => {
        isSaving = false;
        saveMsg = "Save";
        const subLevel = productData.ancestors
          ? productData.ancestors.length
          : 0;
        // cat['sub_level'] = subLevel;
        productData["parent"] = subLevel
          ? productData.ancestors[subLevel - 1]
          : null;
  ...
  updateProduct = async () => {
    ...
    this.appMainService
      .updateProduct(updateProductForm, editedProduct._id)
      .then((updatedProduct) => {
  ...
  editProduct = (editedProduct) => {
    ...
    const editedIndex = this.state.allProducts.findIndex(
      (product) => editedProduct._id == product._id
    );
  ...
  editProductStock = async (viewedProductForStock) => {
    ...
    if (viewedProductForStock.no_of_stocks) {
      ...
      await this.appMainService
        .getProductById(viewedProductForStock._id)
        .then((productStockResponse) => {
  ...
  viewProduct = (event, product) => {
    ...
    const newRoute = `/dashboard/products/${product._id}`;
   ...
   *
   * @param {*} stock
   * This method features a product's stock
   *
   */
  featureStock = (stock) => {
    swal
      .fire({
        title: `<small>Feature&nbsp;<b>${stock?.description}</b>?</small>`,
        text: `This would be the displayed details of  ${this.state.viewedProductForStock?.name}.`,
        icon: "warning",
        type: "question",
        showCancelButton: true,
        confirmButtonColor: "#007BFF",
      ...
      .then((result) => {
        if (result.value) {
          let { productStocks } = this.state;
          this.appMainService
            .featureStock(stock)
            .then((featuredStock) => {
              //    const stockIndex = productStocks.findIndex( s=> s._id == stock._id); // use the forEach loop to kill two birds
              let stockIndex;
              productStocks.forEach((s, index) => {
                s.is_featured = false;
                if (s._id == stock._id) {
                  stockIndex = index;
                }
              });
              productStocks.splice(stockIndex, 1, featuredStock);
              this.setState({ productStocks });
              const successNotification = {
                type: "success",
                msg: `${stock.description} is now featured!`,
              };
              new AppNotification(successNotification);
            })
            .catch((error) => {
  ...
  getSearchPath = (keywords, index = 0, searchPath = "") => {
    return index === keywords.length
      ...
      : this.getSearchPath(
          keywords,
          index + 1,
          `${searchPath}${keywords[index].text}|`
        );
  };
  ...
  setImages = async (images, imagesCommand = "") => {
    ...
    if (imagesCommand == "clr") {
      ...
      newStock["is_featured"] = !viewedProductForStock.no_of_stocks;

      // save to backend
      await this.appMainService
        .createProductStock(viewedProductForStock._id, newStock)
        .then((stocksResponse) => {
          createStockForm = {
            quantity: "",
            price: "",
          ...
          productStocks.unshift(stocksResponse); // add stock to product stocks
          viewedProductForStock.no_of_stocks = productStocks.length;
          viewedProductForStock.stocks = productStocks;
          const viewedProductForStockIndex = allProducts.findIndex(
            (p) => p._id == viewedProductForStock._id
          ); //get index
          allProducts.splice(
            viewedProductForStockIndex,
            1,
            viewedProductForStock
          ); //replace at index
          new AppNotification(successNotification);
  ...
  render() {
    const { navigate, newRoute, authUser } = this.state;
    if (navigate) {
    ...
    return (
      <>
        <div className="specific">
          <Modal
            show={this.state.showManageStockModal}
            onHide={() => {
              this.toggleModal("manage_stock");
            }}
            {...this.props}
            id="manage_stock_modal"
            size="xl"
            scrollable={true}
          >
            <Modal.Header closeButton>
              <Modal.Title>
                {this.state.viewedProductForStock?.no_of_stocks
                  ? "Manage stocks for "
                  : "Add stock(s) to "}
                                ...
                                <div className="col-md-12 mt-2">
                                  <div className="row">
                                      ...
                                      <div className="row">
                                        ...
                                        >
                                          ...
                                          <input
                                            ...
                                            name="quantity"
                                            value={
                                              this.state.createStockForm
                                                .quantity
                                            }
                                            onChange={(event) =>
                                              this.handleChange(event, "stock")
                                            }
                                            onBlur={handleBlur}
                                            required
                                          />
                                        ...
                                        >
                                          ...
                                          <input
                                            ...
                                            name="price"
                                            value={
                                              this.state.createStockForm.price
                                            }
                                            onChange={(event) =>
                                              this.handleChange(event, "stock")
                                            }
                                    ...
                                    <div className="col-md-8 col-sm-12">
                                      <StockImagesUpload
                                        imagesCommand={this.state.imagesCommand}
                                        setImages={(images, imagesCommand) =>
                                          this.setImages(images, imagesCommand)
                                        }
                                      />
                                    </div>
                                  </div>
                              ...
                              {this.state.productStocks.length ? (
                                    ...
                                    <td>
                                      <Dropdown key={stock._id}>
                                        ...
                                        <Dropdown.Menu>
                                          ...
                                          >
                                            <i className="nav-icon i-Pen-2 text-success font-weight-bold">
                                              {" "}
                                            </i>{" "}
                                            Edit
                                          </Dropdown.Item>
                                ...
                                <tr>
                                  <td colSpan="6" className="text-center">
                                    No stocks for{" "}
                                    {this.state.viewedProductForStock?.name}
                                  </td>
                                </tr>
                              )}
                            </tbody>
            ...
            <Modal.Footer>
              <button
                className="btn btn-secondary_custom mr-3"
                onClick={() => {
                  this.toggleModal("manage_stock");
                }}
              >
                    ...
                    <Modal.Body>
                      <div className="form-row">
                        ...
                        <div
                          className={utils.classList({
                            "col-md-12 mb-2": true,
                            "valid-field": !errors.keywords && touched.keywords,
                            "invalid-field":
          ...
          <div className="float-right">
            ...
            >
              <i className="i-Add"></i> Create Product
            </Button>
          </div>
                        ...
                        {this.state.allProducts.length ? (
                              ...
                              >
                                  ...
                                  <>
                                    <td colSpan="2">
                                      <Link
                                        ...
                                        to={`/dashboard/users/${product?.owner?._id}`}
                                        className="underline"
                                        title={product?.owner?.phone_no}
                                      >
                                ...
                                <td>
                                  <div className="btn-group">
                                    <button
                                      disabled={product?.fetching_stock}
                                      onClick={() =>
                          ...
                          <tr>
                            <td
                              className="text-center"
                              colSpan={`${
                                authUser?.user_type == "ADMIN" ? "13" : "11"
                              }`}
                            >
                              <FetchingRecords
